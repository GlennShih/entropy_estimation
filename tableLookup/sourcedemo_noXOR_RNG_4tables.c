...
int main(int argc, char **argv)
{
	...
	/* Ensure we have at least one argument after the program name */
        if (argc < 7) {
                fprintf(stderr, "Usage: %s  <trace file> <packet count> <array file1> <array file2> <array file3> <array file4>\n", argv[0]);
                return 1;
        }
    ...
    uint32_t lineA=0, lineB=0, lineC=0, lineD=0;
	uint32_t i=0,Y=0;
	double z[K_VALUE]={0.0}, tmp2=0.0;
    double lookup_tableA[131072], lookup_tableB[131072], lookup_tableC[131072], lookup_tableD[131072];
	uint32_t int_ip,tmp;
	...
	fileinC = fopen(argv[5],"r");
	fileinD = fopen(argv[6],"r");    
    ...
    while(fscanf(fileinC, "%lf", &lookup_tableC[lineC]) != EOF)
    {
        lineC++;
    } 
    fclose(fileinC);
    
    while(fscanf(fileinD, "%lf", &lookup_tableD[lineD]) != EOF)
    {
        lineD++;
    } 
    fclose(fileinD);
